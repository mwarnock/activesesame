module ActiveSesame
  module SesameRepository
    require 'rest-open-uri'
    require 'uri'

    def SesameRepository.append_features(someClass)
      @@repository_uri = "http://localhost:8111/sesame/repositories"
      @@triple_store_id = "study-stash"
...
#      end
	...
	uri_with_quacks.slice(1,uri_with_quacks.length-2)
      ...
      def someClass.set_base_uri(base_uri)
        self.base_uri = "BASE <#{base_uri}>"
      end

      def someClass.simple_rest_methods(*method_names)
        method_names.each do |name|
	  new_name = "someClass.#{name.to_s}"
          define_method(new_name) { return rest_call(name) }
        end
      end

      someClass.simple_rest_methods :size, :contexts, :namespaces

      def someClass.rest_call(method_name, args={})
	...
	#puts @@location + slash + method_name.to_s + vars_if_get
      ...
      def someClass.encode_sparql(query)
        URI.encode(query).gsub("?","%3f").gsub("/","%2f").gsub(":","%3a").gsub("\\","5C")
      end

      def someClass.hash_to_get(hash)
        (hash.inject("?") {|total,tuple| total += "#{tuple[0]}=#{tuple[1]}&"}).chop
      end

      super #Necissary Call for Append Features
    end
