module ActiveSesame
  module Repository
    ...
    def Repository.append_features(klass)
...
#      end
      ...
      def klass.set_base_uri(base_uri)
        self.base_uri = "BASE <#{base_uri}>"
      end

      def klass.simple_rest_methods(*method_names)
        method_names.each do |name|
	  new_name = "klass.#{name.to_s}"
          define_method(new_name) { return rest_call(name) }
        end
      end

      klass.simple_rest_methods :size, :contexts, :namespaces

      def klass.rest_call(method_name, args={})
	...
	#puts @@location + slash + method_name.to_s + vars_if_get
      ...
      def klass.encode_sparql(query)
        URI.encode(query).gsub("?","%3f").gsub("/","%2f").gsub(":","%3a").gsub("\\","5C")
      end

      def klass.hash_to_get(hash)
